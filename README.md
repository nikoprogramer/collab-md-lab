...
Wrzuć projekt na GitHub (`.gitignore` wyklucza `node_modules/`, `dist/`, `users.db` i `files/`):
...
### 3. Persistent Volume (konieczny!)

Bez wolumenu dane przepadają przy każdym restarcie kontenera.

1. W projekcie Railway: **+ Add a volume**
2. Mount path: `/data`
3. W zakładce **Variables** dodaj zmienną środowiskową:
   ```
   DATA_DIR=/data
   ```

Serwer zapisze tam `users.db` i katalog `files/`.

### 4. Zmienne środowiskowe

| Zmienna | Wymagana | Opis |
|---------|----------|------|
| `DATA_DIR` | **Tak** | Ścieżka do persistent volume, np. `/data` |
| `PORT` | Nie | Railway ustawia automatycznie |
| `NODE_ENV` | Nie | Railway ustawia `production` automatycznie |

### 5. Domena

Railway przydziela domenę automatycznie (`*.up.railway.app`).  
Własną domenę podpinasz w **Settings → Networking → Custom Domain**.
